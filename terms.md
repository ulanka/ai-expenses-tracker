
---

# **Terms of Service – AI Expenses Tracker**

**Effective date:** January 2026

These Terms of Service (“Terms”) govern your use of the **AI Expenses Tracker** mobile application (the “App”), operated by **Ulan Korabay** (“we”, “us”, “our”).

By downloading, accessing, or using the App, you agree to these Terms.

If you do not agree, do not use the App.

---

## 1. The Service

AI Expenses Tracker is a personal finance application that allows users to:

* Record expenses and income
* Upload receipt photos
* View financial reports
* Use AI-powered extraction and summaries
* Access subscription-based features

The App is provided for personal financial tracking only.

---

## 2. Accounts

To use the App, you must create an account.

You are responsible for:

* Maintaining the confidentiality of your login credentials
* All activity under your account

You must provide accurate and up-to-date information.

---

## 3. User Content

You retain ownership of:

* Financial records
* Receipts
* Notes
* Photos
* Any data you enter

By using the App, you grant us a limited license to:

* Store
* Process
* Analyze
* Display

your content solely to provide the App’s functionality.

We do not sell your content.

---

## 4. AI Features

The App uses AI (Gemini) to:

* Extract data from receipts
* Summarize spending
* Generate insights

AI results are automated and may contain errors.
You are responsible for reviewing and verifying all financial data before relying on it.

We are not responsible for financial losses caused by incorrect AI output.

---

## 5. Subscriptions and Payments

Some features require a paid subscription.

Payments are processed by **Apple App Store**.

We do not control:

* Billing
* Refunds
* Cancellations

These are handled by Apple under their App Store terms.

---

## 6. Acceptable Use

You agree not to:

* Upload illegal content
* Attempt to access other users’ data
* Reverse-engineer the app
* Abuse, scrape, or overload the system
* Use the App for unlawful purposes

We may suspend or terminate accounts that violate these rules.

---

## 7. Data and Privacy

Your use of the App is governed by our **Privacy Policy**.
By using the App, you consent to the collection and processing of data as described there.

---

## 8. Availability and Changes

We may:

* Modify features
* Add or remove functionality
* Temporarily suspend the App
* Update these Terms

Continued use means you accept the updated Terms.

---

## 9. Disclaimer

The App is provided “as is”.

We do not guarantee:

* Financial accuracy
* Continuous availability
* Error-free operation

AI Expenses Tracker is not a bank, financial advisor, or accounting service.

---

## 10. Limitation of Liability

To the maximum extent allowed by law, we are not liable for:

* Financial losses
* Data loss
* Incorrect AI results
* App downtime
* Indirect or consequential damages

---

## 11. Termination

You may stop using the App at any time.

We may suspend or terminate your account if:

* You violate these Terms
* You misuse the service
* Required by law

---

## 12. Governing Law

These Terms are governed by the laws of the developer’s jurisdiction.

---

## 13. Contact

For support or legal inquiries:

**Email:** [tinges_0_tattles@icloud.com](mailto:tinges_0_tattles@icloud.com)

